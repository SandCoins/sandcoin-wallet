...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SandCoins
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SandCoins
    // Mac: ~/Library/Application Support/SandCoins
    // Unix: ~/.sandcoins
#ifdef WIN32
    // Windows
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 35 * 60) // CasinoCoin: changed maximum adjust to 35 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
